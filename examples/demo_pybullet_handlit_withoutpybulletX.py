...
def load_robot(urdf_path,base_position,base_orientation,use_fixed_base):
    robot_id = p.loadURDF(fileName=urdf_path,useFixedBase=use_fixed_base,basePosition=base_position,baseOrientation=base_orientation)
    return robot_id
...
@hydra.main(config_path="conf", config_name="handlit")
def main(cfg):
    # Initialize digits
    ...
    robot_id = load_robot(**cfg.allegro)


    # Add cameras to tacto simulator
    digits.add_camera(robot_id, cfg.digit_link_id_allegro)
